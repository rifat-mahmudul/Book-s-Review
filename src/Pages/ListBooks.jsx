    ...
    return (
        <div className="max-w-[90%] xl:max-w-[1200px] mx-auto">
            ...
            <div className={`flex items-center overflow-x-auto overflow-y-hidden sm:justify-start flex-nowrap`}>
                <Link to='' onClick={()=> {setIndex(0); setShowRead(true); setShowWishlist(false)}} className={`flex items-center flex-shrink-0 px-5 py-3 space-x-2 rounded-t-lg border-black ${index === 0 ? 'border-2 border-b-0 ' : 'border-b-2'}`}>
...

            {
                ...
                &&
                <div className="pb-10">
                {books.map((book, index) => <Read key={index} book={book}></Read>)}
                </div>
            }

            {
                ...
                &&
                <div className="pb-10">
                {wishlist.map((book, index) => <ShowWishlist key={index} book={book}></ShowWishlist>)}
                </div>
            }
        </div>
    )
